...
> **Definition 1 (Realizability):** Let $W \subset \mathbb{R}^d$ be the set of all parameters. If there exists $w_0 \in W$ such that $q(x) = p(x|w_0)$ almost surely, then $q(x)$ is said to be realizable by $p(x|w)$. For a given pair of true distribution and statistical model, the set of true parameters is defined by
> $$
...
$$
W_{00} = \{w \in W: \int q(x)\log\dfrac{q(x)}{p(x|w)}dx = 0\}
$$
...
L(w) = S + K(q(x)\,\|\,p(x|w))
$$
where $S = -\int q(x)\log q(x)dx$ is the entropy of the true distribution and $K(\cdot\,\|\,\cdot)$ denotes the KL divergence. Hence, if $q(x)$ is realizable by a statistical model, then the average log loss function is minimized iff $w \in W_{00}$, and the minimum value is the entropy of the true distribution.
...
> **Definition 2 (Regularity):** For a given pair of $q(x)$ and $p(x|w)$, let $W_0 = \{w \in W: L(w) = \min_{w'}L(w')\}$, which is called the set of optimal parameters for the minimum average log loss. $q(x)$ is said to be regular for $p(x|w)$ if the following three conditions hold:
>
> 1) $W_0$ is a singleton set.
> 2) $w_0 \in W_0$ is in the interior of $W$ ($W$ is equipped with the subspace topology from $\mathbb{R}^d$).
...
> **Definition 3 (Essential Uniqueness):** Assume that $W_0$ is not an empty set. If there exists a unique probability density function $p_0(x)$ such that for arbitrary $w_0 \in W_0$, $p(x|w_0) = p_0(x)$, then it is said that the optimal probability density function is essentially unique.
...
> **Definition 4 (Relatively Finite Variance of Log Density Ratio Function):** For a given pair $w_0 \in W_0$ and $w \in W$, the log density ratio function is defined by
>
> $$
...
Lemma: Assume that $W$ is a compact set and that $q(x)$ is realizable by $p(x|w)$ and that the log density ratio function $f(x, w) = \log \dfrac{q(x)}{p(x|w)}$ is a continuous function of $(x, w)$. If there exist $c_1, c_2 > 0$ such that for an arbitrary $w \in W$,
...
Assume now that $f(x, w_0, w) = \log \frac{p(x|w_0)}{p(x|w)}$ has a relatively finite variance. Then the optimal probability density is essentially unique, so we may denote the log density ratio by $f(x, w)$.

See that $L(w) = E_X \left[ \log \frac{1}{p(X|w)} \right] = E_X \left[ \log \frac{p(X|w_0)}{p(X|w)} \right] - E_X [\log p(X|w_0)]$

$$= E_X [f(X, w)] - E_X [\log p(X|w_0)]$$

$$= E_X [f(X, w)] + L(w_0)$$

We define the normalized average log loss function as $E_X[f(X, w)] = K(w)$.
A key thing to note in all the definitions is that we will be rewriting them in terms of $f(x, w)$. The formal advantage is explained in a short while, but the understanding is that we want to remove the entropy of the optimal probability density and consider only the KL divergence between $p_0(x)$ and the model $p(x|w)$, as that is what we want to analyse (how that changes over time and what are the asymptotics).

(For this to make sense, note that $E_X [f(X, w)] = K(p_0 \,\|\, p(\cdot|w)) = K(w)$, which equals $K(q \,\|\, p(\cdot|w))$ in the realizable case.)
...
Furthermore, a key property of our normalized function is that
...
Thus, $Z_n = \prod_{i=1}^n p_0(X_i) \, Z_n^{(0)} = \exp(-n L_n(w_0)) Z_n^{(0)}$.
...
| Observable | Definition | Normalized version |
| :--- | :--- | :--- |
| Generalization loss $G_n$ | $-E_X [\log p(X \mid X^n)]$ | $-E_X [\log E_w [\exp(-f(X,w))]]$ |
| Training loss $T_n$ | $-\frac{1}{n} \sum_{i=1}^n \log p(X_i \mid X^n)$ | $-\frac{1}{n} \sum_{i=1}^n \log E_w [\exp(-f(X_i,w))]$ |
| Cross validation loss $C_n$ | $\frac{1}{n} \sum_{i=1}^n \log E_w \left[ \frac{1}{p(X_i \mid w)} \right]$ | $\frac{1}{n} \sum_{i=1}^n \log E_w [\exp(f(X_i,w))]$ |
| WAIC $W_n$ | $T_n + \frac{1}{n} \sum_{i=1}^n V_w [\log p(X_i \mid w)]$ | $T_n^{(0)} + \frac{1}{n} \sum_{i=1}^n V_w [f(X_i,w)]$ |
...
Note that this is the $\alpha$-power mean of $p(x|w)$ with respect to the posterior distribution.
...
Note that $F_X(\alpha) = g_x(-\alpha) + \alpha \log p_0(x)$.

The point of introducing the CGF is to be able to do a Taylor expansion here.

Doing a Taylor approximation near 0 (here $g(0) = \log E_w[1] = 0$),
...
Summing over the training points and writing $R_n$ for the accumulated remainder terms, we get
...
We need to clarify the behavior of these random variables, look at higher order terms. The normalized versions of these all converge to 0 in probability, removing the effects of $L_n(w_0)$ and $L(w_0)$ and focusing on what is necessary.
...
Proof: By Taylor's theorem with the Lagrange remainder, given $\alpha$, there exists $\alpha^*$ s.t. $|\alpha^*| \le |\alpha|$ and
...
1. An arbitrary triple of $(q(x), p(x|w), \varphi(w))$ is fixed. The set of parameters is denoted $W$, and $X_1, \dots, X_n \overset{\text{i.i.d.}}{\sim} q(x)$.
...
3. Check that the log density ratio function has a relatively finite variance. Then we have essential uniqueness.
...
Calculate $E_w[f(x,w)]$ and $V_w[f(x,w)]$. From these we obtain

$$E_w[K(w)] = E_X E_w[f(X,w)]$$
$$E_w[K_n(w)] = \frac{1}{n} \sum_{i=1}^n E_w[f(X_i,w)]$$

and the empirical variance term $\frac{1}{n} \sum_{i=1}^n V_w[f(X_i,w)]$, which estimates $E_X V_w[f(X,w)]$.
